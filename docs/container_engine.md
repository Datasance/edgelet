...
- Image, args, env vars (including injected `SELFNAME` and `TZ`)
...
The iofog engine persists runtime state as containerd container labels so it can be recovered after an agent restart without querying the controller:

| Label | Value | Purpose |
|-------|-------|---------|
| `iofog-ms` | microservice UUID | Primary lookup key; used by all listing functions |
| `iofog-name` | microservice name | Human-readable identification |
| `iofog-app` | application name | Application grouping |
| `iofog-sandbox-id` | CRI sandbox ID | Teardown (StopPodSandbox / RemovePodSandbox) |
...
| `iofog-ports` | JSON `[]*PortMapping` | Stored at creation; compared in drift detection |
| `iofog-log-size` | int64 | Log size limit |
| `iofog-healthcheck` | JSON `*Healthcheck` | Healthcheck config; survives controller disconnect |
| `iofog-hostnet` | `"true"` or absent | Host-network mode flag (see below) |

### Why `iofog-hostnet` Exists

For CRI-managed containers, `c.Spec(ctx).Linux.Namespaces` always contains a network namespace entry with a **non-empty path** — even for host-network containers. This is because the workload container's spec is generated by the CRI plugin to share the sandbox's network namespace, and the sandbox's namespace is referenced as `/proc/<SANDBOX_PID>/ns/net`. Even though this path resolves to the host network namespace when the sandbox uses `NamespaceMode_NODE`, the path string is non-empty, making OCI-spec-based detection unreliable.

The `iofog-hostnet` label is written at creation time with the exact value of `ms.HostNetworkMode` and read back in `AreMicroserviceAndContainerEqual`, bypassing the spec entirely:

```go
// At creation (engine.go — CreateContainer)
if ms.HostNetworkMode {
    containerConfig.Labels[labelHostNet] = "true"
}

// At drift check (engine.go — AreMicroserviceAndContainerEqual)
storedHostNet := info.Labels[labelHostNet] == "true"
if ms.HostNetworkMode != storedHostNet {
    // drift detected → schedule UPDATE
}
```
...
func (e *Engine) recoverState() {
    ...
    for _, c := range cs {
        ...
        info, _ := c.Info(ctx)
        st := stateFromLabels(info.Labels)  // reads iofog-ip, iofog-sandbox-id, etc.
        e.store.set(c.ID(), st)
    }
}
```
