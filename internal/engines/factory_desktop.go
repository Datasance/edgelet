...
func NewContainerEngine(engineType string, cfg engine.EngineConfig) (engine.ContainerEngine, error) {
	...
	case constants.EngineEdgelet:
		return nil, fmt.Errorf("container engine %q is not available on this platform (use docker or podman)", engineType)

	default:
		return nil, fmt.Errorf("unknown container engine type %q: must be one of docker, podman", engineType)
	}
}
