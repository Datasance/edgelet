...
// StartPull begins an async pull for one model name. One in-flight pull per name;
...
func (m *Manager) StartPull(name string) (*PullOperation, error) {
	...
	logging.LogInfo(moduleName, fmt.Sprintf("model pull started name=%s operationId=%s generation=%d", name, started.OperationID, row.Generation))
...
func (m *Manager) runPull(parent context.Context, operationID, name string) {
	...
	if err := m.doPull(ctx, operationID, name); err != nil {
		...
		logging.LogWarn(moduleName, fmt.Sprintf("model pull failed name=%s operationId=%s err=%v", name, operationID, err))
		return
	}
	m.succeedOp(operationID)
	logging.LogInfo(moduleName, fmt.Sprintf("model pull succeeded name=%s operationId=%s", name, operationID))
}
...
func (m *Manager) doPull(ctx context.Context, operationID, name string) error {
	...
	if err := m.setState(row.Name, models.ModelStatePulling, ""); err != nil {
		return err
	}
	...
	if skip, skipErr := m.canSkipPull(row); skipErr != nil {
		...
		if err := m.markObservedReady(row, nil); err != nil {
			return err
		}
	...
	_, _ = modelpull.CleanupStaleIncomplete(m.modelsRoot, modelpull.StaleIncompleteAge, m.now())
	...
	if result == nil {
		return errors.New("model pull returned no result")
	}
	if err := recordIdentity(result.ManifestPath, desiredKey); err != nil {
		return err
	}
	if err := m.markObservedReady(row, result); err != nil {
		return err
	}
	m.setProgress(operationID, 100)
	return nil
...
func (m *Manager) pullerFor(reg *models.Registry) (Puller, error) {
	...
	case models.RegistryTypeOCI:
		...
		if m.oci == nil {
			return nil, errors.New("oci model pull adapter is not configured")
		}
		return m.oci, nil
	case models.RegistryTypeHF:
		...
		if m.hf == nil {
			return nil, errors.New("huggingface model pull adapter is not configured")
		}
		return m.hf, nil
	default:
		return nil, fmt.Errorf("registry %d has type %q; model pull requires type oci or hf", reg.ID, reg.NormalizedType())
	}
}
...
func (m *Manager) setState(name, state, lastError string) error {
	...
	nowSec := m.now().Unix()
	if row.State != state {
		row.LastTransitionAt = nowSec
	}
	row.State = state
	row.LastError = lastError
	row.LastReconcileAt = nowSec
...
func (m *Manager) markFailed(name string, pullErr error) error {
	...
	nowSec := m.now().Unix()
	row.State = models.ModelStateFailed
	if pullErr != nil {
