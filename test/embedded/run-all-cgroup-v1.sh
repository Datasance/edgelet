...
#   --vm-name=NAME    Lima VM name (default: iofog-test-v1)
...
LIMA_YAML="${SCRIPT_DIR}/lima-ubuntu-v1.yaml"
...
echo ""
echo -e "${_BOLD}======================================================================${_RESET}"
echo -e "${_BOLD}  Edgelet Embedded — Hybrid cgroup v1 Integration Suite${_RESET}"
...
if [[ "${SKIP_START}" == "false" ]]; then
    log_step "Step 3/5: Starting Lima VM '${VM_NAME}' (hybrid cgroup v1)"
...
log_step "Step 5/5: Running cgroup v1 integration tests"
set +e
"${SCRIPT_DIR}/vm-test-cgroup-v1.sh" --vm-name="${VM_NAME}"
TEST_STATUS=$?
set -e
...
if [[ "${DELETE_VM}" == "true" ]]; then
    ...
    log_info "  Re-run gate only: ./test/embedded/vm-test-cgroup-v1.sh --vm-name=${VM_NAME}"
...
if [[ "${OVERALL_STATUS}" -eq 0 ]]; then
    echo -e "${_GREEN}  CGROUP V1 SUITE PASSED${_RESET}${_BOLD}  (${ELAPSED}s)"
else
    echo -e "${_RED}  CGROUP V1 SUITE FAILED${_RESET}${_BOLD}  (${ELAPSED}s)"
fi
echo -e "${_BOLD}======================================================================${_RESET}"
echo ""
