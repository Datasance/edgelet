#!/usr/bin/env bash
# unified test orchestrator step — build edgelet image, then nested Docker smokes on the Mac host.
#
# Usage:
#   ./test/embedded/run-all-nested-docker.sh
#   ./test/embedded/run-all-nested-docker.sh --skip-build
#   ./test/embedded/run-all-nested-docker.sh --image=edgelet:local --arch=arm64
...
Builds the scratch edgelet image (Dockerfile), then runs nested
...
Options:
  ...
  --image=NAME       Docker tag to build and test (default: edgelet:local)
...
build_image() {
    ...
    log_step "Building edgelet image (${IMAGE}, platform=${_platform})"
